...
int main(void){
  double start, finish;
  double t1=157.219875;// time required for serial execution
  start=omp_get_wtime();
  init();
  int i, j, k; //iterators
  ...
  #pragma omp parallel default(none) shared(p,feedback,number_of_combinations,top10_combinations,top10_averages,top10_std_dev) private(selected_policies,combination,j,k,i,current_avg,current)
  {
  ...
  printf("\nTotal execution time: %f seconds on %d cores\n",finish-start,p);
  printf("\nSpeed up: %f\n",t1/(finish-start));
  printf("\nEfficiency: %f percent\n",((t1/(finish-start))/p)*100);
  return 0;
}
